 ...
 *
 * This task does the following:
 *     1. Gets the source folder for JS public files
...
gulp.task(
  ...
  gulp.parallel(gulp.series('adminJS', 'publicJS'), 'styles', 'images', browsersync, () => {
    ...
    gulp.watch(config.watchJsCustom, gulp.series('publicJS', reload)); // Reload on publicJS file changes.
    gulp.watch(config.imgSRC, gulp.series('images', reload)); // Reload on publicJS file changes.
  })
);
